...
{
    public class S3DataSyncStorage<TDataItem> 
    {
        ...
        public S3DataSyncStorage(string bucketName)
        {
        ...
        public IEnumerable<TDataItem> GetDataPage(string syncId, int page)
        {
            ...
            var stream = this.ReadObjectData(syncId, $"{page}.json");
        ...
        {
            var output = "";
            var stream = this.ReadObjectData(syncId, "index.json");
            using (StreamReader reader = new StreamReader(stream))
            {
        ...
        {
            return this.ReadObjectData(sessionId, $"{page}.json");
        }
        ...
        private Stream ReadObjectData(string syncId, string keyName)
        {
            try
            {
                var request = new GetObjectRequest
                {
                    BucketName = _bucketName,
                    Key = $"{syncId}/{keyName}",
                    ByteRange=new ByteRange(0,230888890)
                };
